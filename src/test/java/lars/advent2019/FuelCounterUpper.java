...
class FuelCounterUpper {
    ...
    @Test
    void solution2() {
        long sum = INPUT.lines().mapToLong(((Long::parseLong))).map(this::calculateRequiredFuelIncludingFuelForFuel).sum();

        assertThat(sum).isEqualTo(5062623);
    }

    private long calculateRequiredFuel(long i) {
        long l = i / 3;
        double floor = Math.ceil(l);
        return Math.max(0, (long) (floor - 2));
    }

    private long calculateRequiredFuelIncludingFuelForFuel(long mass) {
        if (mass <= 0) {
            return 0;
        } else {
            long fuel = calculateRequiredFuel(mass);
            return fuel + calculateRequiredFuelIncludingFuelForFuel(fuel);
        }
    }

    private static final String INPUT = "104489\n"
            + "69854\n"
            + "93424\n"
            + "103763\n"
            + "119636\n"
