...
class SampleHorseData {
  ...
  /**
   * Sample Horse data which you could adapt for your tests. Horses may have more or less parameters
   * than this and there may be more or less rows in the table.
   */
  ...
  /**
   * Sample filter metadata that filters on two fields Breed must equal 'Thoroughbred' and age must
   * equal 3
   */
  static List<FilterMetadata> GetSampleFilters() {
    return Arrays.asList(new FilterMetadata("Breed", "Thoroughbred"), new FilterMetadata("Age", 2));
  }

  /**
   * Sample sort metadata. You only ever sort on one field. In this example you should sort by horse
   * height descending
   */
  ...
  /**
   * Sample pagination metadata. In this example, each page of data should have 3 rows and the first
   * record that should be shown on the page is the one at index 4.
   */
