...
class ChatHistoryService {
  ...
  public ChatHistoryService(S3Client s3Client) {
    this(s3Client, Clock.systemDefaultZone());
  }

  public ChatHistoryService(S3Client s3Client, Clock clock) {
    this.s3Client = s3Client;
    this.clock = clock;
  }

  void whoSaidWhatWhen(String who, String what) {
    whoSaidWhatWhen(who, what, LocalDateTime.now(clock));
  }
  ...
  List<String> history(String who) {
    try {
      ...
      return allContent.lines().filter(getCorrectStringPredicate(who)).toList();
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  }
