 ...
 * a time, using the method <code>lines</code>, or a word at a time, using the method <code>words
 * </code>. These strings can then be iterated over using a <code>for</code> loop.
 *
 * <p>Example usage:
 *
 * <PRE>
 ...
 * </PRE>
 *
 * <p>If each line of the file represents separated data values, because its a CSV file, then the
 * user can get a <code>getCSVParser</code> object to access that data more directly, using one of
 * the <code>getCSVParser</code> methods.
 *
 * <p>Example CSV usage:
 *
 * <PRE>
 ...
 * </PRE>
 *
 * <p>This software is licensed with an Apache 2 license, see
...
public class FileResource {
   ...
   *
   * <p>Useful, for example, when used in conjunction with the <code>DirectoryResource</code> class.
   *
   ...
   * parameter.
   *
   * <p>The named file should be on the current class path to be found.
   *
   ...
   *
   * <p>If the user wants to change the contents of the open file by using the method <code>write
   * </code> to add new strings to it, pass <code>true</code> as the second parameter. Otherwise it
   * is assumed the user will only iterate over the existing contents of the file.
   *
   ...
   *
   * @return an <code>Iterable</code> that will allow access to contents of opened file one line at
   *     a time.
   */
   ...
   * This means any form of spaces, like tabs or newlines, can delimit words.
   *
   * @return an <code>Iterable</code> that will allow access to contents of opened file one word at
   *     a time.
   */
   ...
   *
   * <p>Each line of the file should be formatted as data separated by commas and with a header row
   ...
   *
   * <p>Each line of the file should be formatted as data separated by commas and with/without a
  ...
  /**
   * Allows access to the column names of the header row of a CSV file (the first line in the file)
   * one at a time. If the CSV file did not have a header row, then an empty <code>Iterator</code>
   * is returned.
   *
