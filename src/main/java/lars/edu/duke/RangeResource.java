...
public class RangeResource {
  ...
  /**
   * Create a <code>RangeResource</code> object, starting at start and going up to but not including
   * end, [start - end), that increments by 1.
   *
  ...
  /**
   * Create a <code>RangeResource</code> object, starting at start and going up to but not including
   * end, [start - end), that increments by the amount passed as a parameter.
   *
   * @param start the first value in the range, included as one of the values
   * @param end when to stop the range, not included as one of the values
  ...
  public RangeResource(int start, int end, int increment) {
    ...
    if (end < start && increment > 0) {
      throw new ResourceException(
          "RangeResource: invalid increment, cannot be positive when end ("
              + end
              + ") is less than start ("
              + start
              + ")");
    ...
    if (end > start && increment < 0) {
      throw new ResourceException(
          "RangeResource: invalid increment, cannot be negative when end ("
              + end
