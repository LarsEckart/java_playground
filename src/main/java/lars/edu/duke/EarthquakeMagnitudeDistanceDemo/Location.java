 ...
 * guaranteed to have a valid latitude, longitude, and timestamp (both UTC time and elapsed
...
public class Location {
   ...
   * 2 or greater, the initial bearing is stored in results[1]. If results has length 3 or greater,
   * the final bearing is stored in results[2].
   *
   ...
   * can jump forwards or backwards unpredictably. So always use {@link #getElapsedRealtimeNanos}
   * when calculating time deltas.
   *
   * <p>On the other hand, {@link #getTime} is useful for presenting
   * a human readable time to the user, or for carefully comparing location fixes across reboot or
   * across devices.
   *
   * <p>All locations generated by the {@link LocationManager}
   * are guaranteed to have a valid UTC time, however remember that the system time may have changed
   * since the location was generated.
   *
   ...
   * {@link android.os.SystemClock#elapsedRealtimeNanos}, to calculate the age of a fix and to
   * compare Location fixes. This is reliable because elapsed real-time is guaranteed monotonic for
   * each system boot and continues to increment even when the system is in deep sleep (unlike
   * {@link #getTime}.
   *
   * <p>All locations generated by the {@link LocationManager}
   ...
   * and is not related to the device orientation. It is guaranteed to be in the range (0.0, 360.0]
   ...
   * words, if you draw a circle centered at this location's latitude and longitude, and with a
   * radius equal to the accuracy, then there is a 68% probability that the true location is inside
   * the circle.
   *
   * <p>In statistical terms, it is assumed that location errors
   * are random with a normal distribution, so the 68% confidence circle represents one standard
   * deviation. Note that in practice, location errors do not always follow such a simple
   * distribution.
   *
   ...
   * All locations generated by the {@link LocationManager} include an accuracy.
   */
