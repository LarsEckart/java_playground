 ...
 * </PRE>
 *
 * <p>If each line of the web page represents separated data values, because its a CSV file, then
 * the user can get a <code>getCSVParser</code> object to access that data more directly, using one
 * of the <code>getCSVParser</code> methods.
 *
 * <p>Example CSV usage:
 *
 * <PRE>
...
public class URLResource {
   ...
   * parameter.
   *
   * <p>Constructing the object opens a connection and reads the contents of the web page.
   *
   ...
   *
   * <p>Each line of the web page should be formatted as data separated by commas and with a header
   * row to describe the column names.
   *
   * @return a <code>CSVParser</code> that can provide access to the records in the web page one at
   *     a time
   ...
   *
   * <p>Each line of the web page should be formatted as data separated by commas and with/without a
   ...
   *
   * <p>Each line of the web page should be formatted as data separated by the delimiter passed as a
  ...
  public CSVParser getCSVParser(boolean withHeader, String delimiter) {
    if (delimiter == null || delimiter.length() != 1) {
      throw new ResourceException(
          "URLResource: CSV delimiter must be a single character: " + delimiter);
    }
    try {
