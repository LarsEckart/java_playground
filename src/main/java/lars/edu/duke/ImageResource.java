 ...
 *
 * <p>Note, no changes made to the pixels in this image affect the original image opened unless you
 * use the method <code>save</code>.
 *
 * <p>Example usage:
 *
 * <pre>
...
public class ImageResource {
  ...
  /**
   * Create an <code>ImageResource</code> object that represents the file chosen by the user using a
   ...
   *
   * @return the name of the file used to create this image or an empty string if it was created as
   *     a sized image
   */
   ...
   *
   * <p>Useful, for example, when saving the results of changes to this image in a different file
   * than the original.
   *
   ...
   * allow the user to choose a name if no file name set (for example if this image was created as a
   * blank sized image).
   *
  ...
  private int[] pixelsToInts(Pixel[] pixels) {
    ...
    for (int i = 0; i < pixels.length; i++) pix[i] = pixels[i].getValue();
    return pix;
  }
