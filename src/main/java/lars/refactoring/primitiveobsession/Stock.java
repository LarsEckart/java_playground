...
class Stock {
  ...
  public Stock(String company, int shares, double pr) {
    ...
    this.shares = shares;
    if (pr < 0) {
      System.out.println("Share price can't be negative; "
  ...
  void buy(long num, double price) {
    ...
    if (price < 0) {
      System.out.println("The share price can't less than zero. "
  ...
  void sell(long num, double price) {
    if (num > shares) {
      ...
      shares -= num;
      if (price < 0) {
        System.out.println("Share price can't be less than 0"
            + "Transaction is aborted.\n");
      } else {
        shareValue = price;
      }
