 ...
 * 2) Pushing Arguments Out, extract parameter in setter
 * 3) Pushing Return Values, Extract Method on field in getter and then Inline original getter and rename extracted method.
 * 4) Pushing Arguments In
 */
public class Thingy {
...
class PseudoThingyClient {
    ...
    private void doThingyStuff(int thingyId) {
        Gizmo g = new Gizmo();
        g.doTheThing(thingyId); // here also rewrite to new ThingyId(thingyId).intValue() and extract parameter then
    }
}
