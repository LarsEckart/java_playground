...
public class Program {
  ...
  static void Play() {
    ...
    while (changeMade) {
      ...
      // Build a collection (named cellGroups) which maps cell indices into distinct groups(rows /
      // columns / blocks)
      ...
      var rowIndices =
          IntStream.range(0, state.length)
              .mapToObj(
                  index ->
                      new Group(index / 9, "row " + index / 9 + 1, index, index / 9, index % 9))
      ...
      var columnIndices =
          IntStream.range(0, state.length)
              .mapToObj(
                  index ->
                      new Group(
                          9 + index % 9, "column #{index % 9 + 1}", index, index / 9, index % 9))
              .collect(Collectors.groupingBy(t -> t.discriminator));
      ...
      var blockIndices =
          IntStream.range(0, state.length)
              ...
              .map(
                  t ->
                      new Group(
                          18 + 3 * (t.row / 3) + t.column / 3,
                          "block ({tuple.Row / 3 + 1}, {tuple.Column / 3 + 1})",
                          t.index,
                          t.row,
                          t.column))
              .collect(Collectors.groupingBy(t -> t.discriminator));
      ...
      while (stepChangeMade) {
        ...
        // Try to find pairs of digits in the same row / column / block and remove them from other
        // colliding cells
        if (!changeMade) {
          var twoDigitMasks =
              IntStream.range(0, candidateMasks.length)
